...
public class OpcodeTables {
    ...
    static private final HashMap<Integer, Opcode> IntToAllOpcodes = new HashMap<>(MaxOpcodes);
    static private final HashMap<String, Opcode> Mnemocodes = new HashMap<>(MaxOpcodes);
    ...
    public enum OpcodeType {
        NORMAL("Normal"),
        NONPRIVELEGED("NonPriv"),
        PRIVELEGED("Priv"),
        WIDE("Wide");

        private final String printVal;

        OpcodeType(String print) {
            printVal = print;
        }
    ...
    public enum Opcode {
        ...
        private void init(Integer val, String parse, String als, int len, OpcodeType tp) {
            type = tp;
            switch (tp) {
                case NORMAL -> value = val;
                case WIDE -> value = (opc_wide.value << 8) | val;
                case PRIVELEGED -> value = (opc_priv.value * 0xFF) + val;
                case NONPRIVELEGED -> value = (opc_nonpriv.value * 0xFF) + val;
            }
            parseKey = parse;
            alias = als;
            length = len;
        ...
        // JVMS chapter 6.2. Reserved Opcodes
